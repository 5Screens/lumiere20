...
const search = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in search:', error);
    next(error);
  }
};
...
const getAll = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in getAll:', error);
    next(error);
  }
};
...
const getByUuid = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in getByUuid:', error);
    next(error);
  }
};
...
const create = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in create:', error);
    next(error);
  }
};
...
const update = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in update:', error);

    if (error.code === 'P2025') {
      return res.status(404).json({
        error: 'Not found',
        message: `Task with UUID '${req.params.uuid}' not found`,
      });
    }
...
const remove = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in remove:', error);
    next(error);
  }
};
...
const removeMany = async (req, res, next) => {
  try {
    ...
    logger.error('[TASKS CONTROLLER] Error in removeMany:', error);
    next(error);
  }
};
