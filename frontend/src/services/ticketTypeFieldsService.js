...
const BASE_URL = '/ticket_type_fields'
...
export default {
   ...
   */
  async getByTypeUuid(ticketTypeUuid) {
    const response = await api.get(`${BASE_URL}/type/${ticketTypeUuid}`)
   ...
   */
  async reorder(ticketTypeUuid, orderedUuids) {
    const response = await api.post(`${BASE_URL}/type/${ticketTypeUuid}/reorder`, { orderedUuids })
    return response.data
  },
