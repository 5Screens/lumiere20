...
export function useObjectView(options) {
  ...
  const reverseLinkFields = computed(() => {
    ...
    return objectTypeMetadata.value.fields.filter(f => (f.field_type === 'reverse_link' || f.field_type === 'reverse_link_mn' || f.field_type === 'reverse_link_subscription') && f.show_in_detail)
  })
