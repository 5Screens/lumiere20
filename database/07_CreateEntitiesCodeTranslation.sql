...
-- Description: Création de la table entities_code_label dans le schéma translations
-- Date: 2025-03-05

-- Activation du mode transaction
BEGIN;

-- Création de la table entities_code_label
CREATE TABLE translations.entities_code_label (
...
-- Création des index
CREATE INDEX idx_entities_code_label_langue ON translations.entities_code_label (langue);
CREATE INDEX idx_entities_code_label_dates ON translations.entities_code_label (date_creation, date_modification);

-- Création du trigger pour la mise à jour de la date de modification
CREATE TRIGGER update_entities_code_label_date_modification
    BEFORE UPDATE ON translations.entities_code_label
    FOR EACH ROW
    EXECUTE FUNCTION update_date_modification();

-- Création du trigger pour la validation des dates
CREATE TRIGGER validate_entities_code_label_dates
    BEFORE UPDATE ON translations.entities_code_label
    FOR EACH ROW
    EXECUTE FUNCTION validate_modification_date();

-- Insertion des valeurs par défaut pour le français
INSERT INTO translations.entities_code_label (entity_type, langue, libelle) VALUES
