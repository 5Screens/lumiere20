...
CREATE INDEX idx_configuration_items_name ON data.configuration_items (name);
...
CREATE INDEX idx_persons_name ON configuration.persons (last_name, first_name);
...
CREATE INDEX idx_entities_name ON configuration.entities (name);
...
CREATE INDEX idx_ticket_types_translation_lang ON translations.ticket_types_translation (lang);
CREATE INDEX idx_ticket_types_translation_dates ON translations.ticket_types_translation (created_at, updated_at);

CREATE INDEX idx_ticket_status_translation_lang ON translations.ticket_status_translation (lang);
CREATE INDEX idx_ticket_status_translation_dates ON translations.ticket_status_translation (created_at, updated_at);

CREATE INDEX idx_symptoms_translation_lang ON translations.symptoms_translation (lang);
CREATE INDEX idx_symptoms_translation_dates ON translations.symptoms_translation (created_at, updated_at);

-- Indexes pour rel_persons_entities (schéma configuration)
CREATE INDEX idx_rel_persons_entities_person ON configuration.rel_persons_entities (person_uuid);
CREATE INDEX idx_rel_persons_entities_entity ON configuration.rel_persons_entities (entity_uuid);
CREATE INDEX idx_rel_persons_entities_dates ON configuration.rel_persons_entities (created_at, updated_at);

-- Indexes pour tickets (schéma core)
CREATE INDEX idx_tickets_title ON core.tickets (title);
...
CREATE INDEX idx_tickets_type ON core.tickets (ticket_type_code);
CREATE INDEX idx_tickets_status ON core.tickets (ticket_status_code);
...
COMMENT ON INDEX data.idx_configuration_items_name IS 'Index pour la recherche par nom des éléments de configuration';
COMMENT ON INDEX configuration.idx_persons_name IS 'Index pour la recherche par nom et prénom des personnes';
COMMENT ON INDEX configuration.idx_persons_email IS 'Index pour la recherche par email des personnes';
COMMENT ON INDEX configuration.idx_entities_name IS 'Index pour la recherche par nom des entités';
